...
Known facts captured on 2026-09-21 from the current repository only. Ownership, license, permission, and release status remain `待确认` unless the repository contains explicit source documentation for that asset type.

## Automated asset audit

Run `npm run audit:assets` before every release. The audit is read-only and checks raster format, dimensions, EXIF GPS metadata, sensitive EXIF/XMP/text metadata, and whether each file can be decoded. The current authorized audit on 2026-09-21 reported 175 image files, `GPS metadata: 0`, `Sensitive metadata: 0`, and `Unreadable files: 0`. A non-zero result blocks release until the files are reviewed; it does not automatically strip metadata or change assets.
...
| Yance brand mark | `design-assets/yance-brand-master.png`, `public/assets/brand/yance-*`, `public/assets/brand/favicon-*` | AI-generated personal brand artwork selected and approved by site owner on 2026-08-26; small icon is a conservative Y-only derivation | Site owner / AI-assisted generation | Approved for site branding | Approved | Original selected master is source-only and stays outside `public/`; small icon removes dense detail for 16px legibility. |
| Home Hero LXGW WenKai subsets | `public/assets/fonts/lxgw-wenkai-hero-sc.woff2`, `public/assets/fonts/lxgw-wenkai-hero-tc.woff2` | Official `lxgw/LxgwWenKai` and `lxgw/LxgwWenKaiTC` v1.522 regular TTF releases, subset with `scripts/subset-home-hero-fonts.py` using the pinned FontTools toolchain | LXGW WenKai Project Authors | SIL Open Font License 1.1; see the adjacent `OFL-LXGW-WenKai*.txt` files | Approved for this site asset use subject to the license | The two license files apply only to their corresponding derived font assets and do not license the rest of this repository. |
...
- Exception recorded for this release: the site owner explicitly approved publishing the 15 listed public promotional posters and the exact public event facts for Zhou Shen on 2026-09-27 at National Stadium and FFOREVER on 2026-10-06 at National Stadium. This exception does not change the pending copyright, reuse, or venue-rights status.
