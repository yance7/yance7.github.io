...
describe('footer contact data', () => {
  it('publishes only email and GitHub in the locked order', () => {
    ...
    expect(data).toContain("export type FooterContactKey = 'email' | 'github'")
    expect(data).not.toContain('instagram')
    expect(data).not.toContain('x.com')
...
describe('footer localization contract', () => {
  it('keeps identity, navigation, and contact labels synchronized', () => {
    ...
    for (const messages of [zhCNMessages, zhHKMessages, enMessages]) {
      ...
      expect(messages.footer).not.toHaveProperty('instagram')
      expect(messages.footer).not.toHaveProperty('x')
    }
    ...
    expect(types).not.toContain('instagram: string')
    expect(types).not.toContain('x: string')
    expect(types).not.toContain('researchRepos: string')
  })
})

describe('footer component contract', () => {
  it('uses one localized home mark and a semantic two-link contact grid', () => {
    ...
    expect(iconSource).not.toContain("name === 'instagram'")
    ...
    expect(styles).toContain('transition: color 200ms var(--ease-soft)')
    expect(responsive).toMatch(/@media \(max-width: 960px\)[\s\S]*\.foot-contacts[\s\S]*repeat\(2, minmax\(0, 1fr\)\)/)
    expect(responsive).toContain('@media (max-width: 300px)')
    expect(responsive).toContain('.foot-contacts { grid-template-columns: 1fr; }')
    expect(footer).toContain('<span class="foot-contact-arrow" aria-hidden="true">↗</span>')
    expect(footer).not.toContain("contact.external ? '↗' : '→'")
  })
})
