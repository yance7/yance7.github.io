...
在第一轮“上下文出现的移动罗盘”基础上继续收敛阅读节奏：移动端用户主动向下阅读时，固定罗盘不应持续占据内容层；停止滚动或回看时，它应恢复为可发现的导航工具。研究时间轴则需要用非文字的当前位置状态，帮助读者知道自己正在阅读哪一条记录。

### 追加方案

- 移动端 PageCompass 在下滑期间进入 `reading` 状态并淡出；滚动停止后恢复，向上滚动立即恢复。
- 当罗盘内部拥有键盘焦点时，不因滚动方向隐藏，避免打断键盘操作；焦点离开后重新遵循当前滚动状态。
- 桌面端罗盘继续常驻，移动端首屏 quiet 状态、页脚安全间距和 44px 触控目标保持不变。
- 研究时间轴通过视口阅读区标记当前 `.tl-item`，只调整节点、日期和边界的视觉层级，不增加文案、不改变内容顺序或交互语义。
- 当前位置反馈使用颜色、边框和光晕过渡；reduced-motion 下保留状态但移除过渡和呼吸动画。

### 追加验收标准

- 移动端下滑阅读时 PageCompass 不挡住内容；停止滚动后可恢复操作，向上滚动立即可用。
- 罗盘获得焦点后继续可见且焦点不丢失；桌面端现有导航、深链接和回到顶部行为不变。
- 研究时间轴在滚动阅读区内始终只有一个当前条目，切换时不改变布局尺寸或文字换行。
- 不修改 `src/data/`、页面文案、URL、HTML entry 内容、静态资源或 `dist/`。
